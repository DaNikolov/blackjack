...
function addNewPlayer() {
    if (numberOfPlayers === 5) return alert("Maximum number of players has been reached")
...
function startGame() {
    if (numberOfPlayers === 0) return alert("Please add players to the game")
...
function stay() {
    ...
    if (check.length >= 1) return alert(`Player ${utilFunctions.determineCurrentPlayer().name} is next`);
